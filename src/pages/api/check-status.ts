...
export default async function handler(req: NextApiRequest, res: NextApiResponse<CheckStatusApiResponse | string>) {
  if (req.method !== 'GET') {
...
export const searchPassportStatusApi = async (
  ...
  if (!process.env.PASSPORT_STATUS_API_BASE_URI) {
    logger.error('PASSPORT_STATUS_API_BASE_URI must not be undefined, null or empty');
  ...
  if (response.ok) {
    ...
    if (GetCertificateApplicationResponse.length === 0) {
      logger.debug('error 404: searchResult._embedded is empty, passport status not found');
      return res.status(404).send('Passport Status Not Found');
    }

    return res.json(mapToCheckStatusApiResponse(GetCertificateApplicationResponse[0]));
  }

  if (response.status === 404 || response.status === 422) {
    logger.debug(`Check status returned not OK - error ${response.status}: ${response.body}`);
