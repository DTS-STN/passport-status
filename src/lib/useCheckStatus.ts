...
export const useCheckStatus = (
  checkStatusApiRequestQuery: CheckStatusApiRequestQuery,
  queryOptions?: Omit<
    UseQueryOptions<CheckStatusApiResponse | null, ApiError, CheckStatusApiResponse | null>,
    'queryKey' | 'queryFn'
  >,
) => {
  const query = useQuery<CheckStatusApiResponse | null, ApiError, CheckStatusApiResponse | null>({
    ...(queryOptions ?? {}),
    queryKey: ['ps:api:check-status', checkStatusApiRequestQuery],
    queryFn: ({ signal }) => fetchCheckStatus(checkStatusApiRequestQuery, { signal }),
  });
