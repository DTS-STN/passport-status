...
export const CheckStatusNotAcceptable = ({ checkAnotherHandler }: CheckStatusNotAcceptableProps) => {
  const { t } = useTranslation(['status', 'common']);
  return (
    <div id="response-result">
      <AlertBlock page="status-invalid" />
      <h1 id="main-header" data-testid="not-acceptable" className="h1" tabIndex={-1}>
        {t('not-acceptable.header')}
      </h1>
      <div className="max-w-prose">
        <p>{t('not-acceptable.unable-to-process')}</p>
        <h2 className="h2 mt-8 mb-2">{t('not-acceptable.next-steps.header')}</h2>
