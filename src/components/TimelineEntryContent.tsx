...
const TimelineEntryContent = ({ position, type, topText, bottomText, bottomDate, stepIndex }: TimelineEntryContentProps) => {
  ...
  const stepAriaLabel = (() => {
    const bottomLine = type === 'done' && hasDate ? formatDateLong(bottomDate, i18n.language) : bottomText;
  ...
  return (
    <>
      ...
      <div aria-hidden="true" className={`flex flex-col gap-2`}>
        ...
        {type === 'done' && hasDate ? (
          <time dateTime={bottomDate}>{formatDateLong(bottomDate, i18n.language)}</time>
        ) : (
